...
namespace Examples {
	...
	class SettingMembersTests : public ::testing::Test
	{
	...
	*/
	TEST(SettingMembersTests, SetPrivateInstanceMember)
	{
		//Arrange
		auto a = Isolator();
		...
		//Assert
		ASSERT_EQ(10, memberValue);
		// we can also do this from public api but 
		ASSERT_EQ(10, person->GetId());
	...
	/*	In this test, we set the private static m_static member and check
		The value was actually set.
	*/
	TEST(SettingMembersTests, SetPrivateStaticMember)
	{
		//Arrange
		auto a = Isolator();
	...
	/*	In this test, we'll set and then get the private m_array member and check
		The value was actually set.
	*/
	TEST(SettingMembersTests, GetPrivateArray)
	{
		//Arrante
		auto a = Isolator();
		Person* person = new Person();
