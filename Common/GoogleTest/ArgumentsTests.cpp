...
namespace Examples {
	class ArgumentTests : public ::testing::Test
	{
	};
	/*****************************************
	...
	*/
	TEST(ArgumentTests, SetUpAFakeAndIgnoreForAnyAgumentsPassed)
	{
		//Arrange
		auto a = Isolator();
		Person person;
	...
	*/
	TEST(ArgumentTests, UsingSetOutToFakeAnOutArgument)
	{
		//Arrange
		auto a = Isolator();
		Person person;
		...
		//Assert
		ASSERT_TRUE(result.compare(string("HighStreet")) == 0);
	}
	...
	*/
	TEST(ArgumentTests, ValidatingArgumentsInLambdaMethod)
	{
		//Arrange
		auto a = Isolator();
		Person person;
	...
	*/
	TEST(ArgumentTests, ValidatingMultipleArgumentsInLambdaMethod)
	{
		//Arrange
		auto a = Isolator();
		Person person;
