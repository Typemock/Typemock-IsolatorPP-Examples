...
// All faked static methods follow Recursive Fakes and sequncing rules, like faked instance methods
	class FakeStaticMethods : public ::testing::Test
	{
	};
	...
	*/
	TEST(FakeStaticMethods, ReturningAFakeObjectFromAFactoryMethod)
	{
		//Arrange
		auto a = Isolator();
		...
		ASSERT_STREQ("London", Person::GetCityFromAddressFactory());
	}
	...
	*/
	TEST(FakeStaticMethods, FakingAllStaticMethodsOnAType)
	{
		//Arrange
		auto a = Isolator();
		// Fake all methods
		...
		ASSERT_STREQ("", Person::GetCityFromAddressFactory());
